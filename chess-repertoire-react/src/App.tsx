...
import GameMetadataPanel, {
  type GameMetadata,
} from "./components/GameMetadataPanel";
...
const App: React.FC = () => {
  ...
  const convertPGNMovesToHistory = useCallback(
    (pgnMoves: PGNMove[]): Move[] => {
      const moves: Move[] = [];
        ...
        reader.onload = (e) => {
          ...
          if (parsedGame) {
            ...
            for (const move of convertedMoves) {
              if (move.originalIndex !== -1) {
                // Only main line moves
                try {
  ...
  const handleSavePGN = useCallback(() => {
    ...
    const pgnContent = PGNProcessor.generatePGN(
      metadata,
      moves,
      gameState.comments
    );
    PGNProcessor.downloadPGN(
      pgnContent,
      `${metadata.chapterName.replace(/\s+/g, "_")}.pgn`
    );
  }, [metadata, gameState]);
  ...
  const handleExportChessable = useCallback(async () => {
    ...
    try {
      ...
      Object.entries(chapters).forEach(([chapterName, content], index) => {
        const filename = `${String(index).padStart(
          2,
          "0"
        )}_${chapterName.replace(/\s+/g, "_")}.pgn`;
        setTimeout(() => {
  ...
  const handleMetadataChange = useCallback(
    (key: keyof GameMetadata, value: string) => {
      setMetadata((prev) => ({
        ...prev,
        [key]: value,
      }));
